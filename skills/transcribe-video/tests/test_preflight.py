...
def test_captions_only_needs_ffmpeg_and_ytdlp():
    preflight.require_for_mode("captions", _caps(gpu=False))  # must not raise
...
def test_diarize_off_needs_only_ffmpeg():
    preflight.require_for_mode("diarize_off", _caps())  # must not raise
...
def test_diarize_on_needs_only_ffmpeg_no_hf():
    ...
    preflight.require_for_mode("diarize_on", _caps())  # must not raise


def test_diarize_auto_never_blocks():
    preflight.require_for_mode("diarize_auto", _caps())  # must not raise
...
def test_resolution_file_needs_only_ffmpeg():
    preflight.require_for_resolution("file", _caps(yt_dlp=False))
...
def test_report_capabilities_returns_expected_keys():
    rep = preflight.report_capabilities()
    for k in ["ffmpeg", "ffprobe", "yt_dlp", "gpu"]:
        assert k in rep
    # report-only cache keys (model_cached / diarization_weights) were removed — nothing consumed them
    # (the SessionStart display they were wired toward never existed); first-run download cost is now
    # surfaced just-in-time at model load instead.
    assert "model_cached" not in rep and "diarization_weights" not in rep
