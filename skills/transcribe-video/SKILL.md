...
- `{basename}_manifest.json` — structured manifest, validates against `manifest-1.1.schema.json` (1.0 stays frozen for the prior 1.0 manifests)
- `{basename}_frames.md` — flat frames index (timestamp, sharpness, scene-cut flag, image link); the human-readable view of the curated set

A1.0 **curates and stops** — it produces these local inputs for the prosumer compose tier; it does NOT write a guide. All four artifacts are always produced: `frames/` + `transcript.txt` + `manifest.json` + `frames.md`.
...
# Very long video (90 min+) — sample coarser so curation stays fast
transcribe-video "webinar.mp4" --interval-seconds 3
```
